...
insert into PSITOJXP.AE_BEFORE
select before.policy_number,before.orig_policy_number
 ...
 before.coverage_plan_code, before.ISSUE_STATE,before.file_code, before.trx_code,
  before.effective_Dte as PREM_eff_date, before.POLICY_EFFECT_DaTE, before.COMM_OPTION
  ,before.trx_amount, before.policy_status
  ,before.orig_admin_sys,' ' , int(ahf.policy_duration),ahf.premium_category
  ,ahf.user_adj_ind, ahf.PAYMENT_EFFEC_DATE as payment_eff_date
  ...
  from PSITOJXP.PREMs_before before
  ,psitoz01.acr earacr
  ...
  ,psitoz01.agthist ahf
    ...
    where ahf.company_Code = 'JPF'
     ...
     and AHF.USER_ADJ_IND = ' '
     and substr(ahf.AGENT_NUM_EARNED,1,10) = earacr.agent_id
     and substr(ahf.AGENT_NUM_EARNED,11,3) = earacr.agent_profile
	 ...
	 and substr(ahf.AGENT_NUM_EARNED,1,10) = earapr.agent_id
